...
def make_image_prompt(
    ...
    `is_pano` maps to the API's ``PanoDetectionMode``: ``"auto"`` detects valid
    equirectangular panoramas automatically, ``True`` always treats the image as
    a panorama, and ``False`` treats it as a standard image.
    """
    prompt: dict[str, Any] = {
        "type": "image",
        "image_prompt": {
